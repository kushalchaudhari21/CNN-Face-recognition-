...
def autotrain(nnList, xtr, ytr, g_eta, epochs, b, archlabel, xte=None, yte=None ):
    JLtr = []
    JLte = []
    epochL = []
    for e in tqdm(range(1,epochs+1)):
        ...
        for mb in tqdm(range(xTrain.shape[0]//b)):
            batch_x = xTrain[mb*b:(mb+1)*b,:]
            batch_y = yTrain[mb*b:(mb+1)*b,:]
            h = batch_x
            for i in range(len(nnList)-1):
                h = nnList[i].forward(h)
            
            
            Jtr = nnList[-1].eval(batch_y, h)


            #----------------------------------------------------------------------------------------------------------------------

            # Backward pass 
            grad = nnList[-1].gradient(batch_y, h)
...
#------------------------------------------------------BOX1 END--------------------------------------------------------
    ...
    # MODEL 3 -  Change FCL weight initialisations in range(-0.067,0.067)
    # nn3 = [ConvolutionLayer(5), MaxpoolLayer(3,2), FlattenLayer(), FullyConnectedLayer(17*17,10), SoftmaxLayer(), CrossEntropy()]
    # autotrain(nn3, X_train, Y_train, 0.2, 20, 30, "nn3", X_test, Y_test)  

    # MODEL 5 - Change FCL weight initialisations in range(-0.067,0.067), Dropout p is chosen as 0.6
    nn5 = [ConvolutionLayer(5), MaxpoolLayer(5,2), FlattenLayer(), FullyConnectedLayer(16*16,100), TanhLayer(), DropoutLayer(0.6), FullyConnectedLayer(100,10), SoftmaxLayer(), CrossEntropy()]
    autotrain(nn5, X_train, Y_train, 0.003, 75, 30, "nn5", X_test, Y_test) 

    # # MODEL 11 -  Change FCL weight initialisations in range(-0.067,0.067)
    # nn11 = [ConvolutionLayer(5), MaxpoolLayer(3,2), FlattenLayer(), FullyConnectedLayer(17*17,75), ReLuLayer(), FullyConnectedLayer(75,10), SoftmaxLayer(), CrossEntropy()]
    # autotrain(nn11, X_train, Y_train, 0.003, 100, 30, "nn11", X_test, Y_test) 
